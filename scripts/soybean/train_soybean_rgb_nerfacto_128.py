# -*- coding: utf-8 -*-
"""
Created on Sun Mar  9 16:53:50 2025
...
file_path = 'D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanRGB/transforms.json'  # Update with the path to your file
...
for image_name in img_ls_out:
    img_path = os.path.join("D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanRGB/images", image_name)
...
if os.path.exists(checkpoint_path):
    ...
    print(f"No existing {checkpoint_path}; starting RGB training from scratch.")
##
field_optimizer = torch.optim.Adam(
    params=nerfmodel.field.parameters(),  # All parameters in the field
    lr=1e-2,  # Manuscript RGB-stage field/proposal learning rate
...
camera_optimizer = torch.optim.Adam(
    ...
    lr=1e-4,  # Manuscript RGB-stage camera learning rate
    eps=1e-8,  # Default epsilon
    weight_decay=0  # Default weight decay
)
...
data_config.data = Path('D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanRGB')
...
dataManager_config.dataparser.data = Path("D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanRGB")
...
for epoch in range(4000):
    ...
    if loss_out_final_i_0 < best_loss_i:
        ...
        save_checkpoint_multiple(nerfmodel, optimizers, 
                                 epoch, "soybean_rgb_nerfacto_128.pth")
