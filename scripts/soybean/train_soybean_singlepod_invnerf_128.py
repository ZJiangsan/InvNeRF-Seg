# -*- coding: utf-8 -*-
"""
Created on Sun Mar  9 16:59:01 2025
...
class NerfactoModelConfig(ModelConfig):
    ...
    interlevel_loss_mult: float = 100.0
...
file_path = 'D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanSeg_singlePodMASK/transforms.json'  # Update with the path to your file
...
# dirs = np.stack([np.sum([0, 0, -1] * pose[:3, :3], axis=-1) for pose in poses_out.numpy()])
# origins = poses_out[:, :3, -1]
...
for image_name in img_ls_out:
    img_path = os.path.join("D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanSeg_singlePodMASK/images", image_name)
...
##
near_plane = 0.05#nerfcfig.near_plane  # e.g., 0.05
far_plane = 2 #nerfcfig.far_plane    # e.g., 10.0
...
checkpoint_path = "soybean_rgb_nerfacto_128.pth"
if not os.path.exists(checkpoint_path):
    raise FileNotFoundError(f"Required RGB checkpoint not found: {checkpoint_path}. Run train_soybean_rgb_nerfacto_128.py first.")
...
field_optimizer = torch.optim.Adam(
    ...
    lr=1e-4,  # Manuscript fine-tuning field/proposal learning rate
...
camera_optimizer = torch.optim.Adam(
    ...
    lr=1e-6,  # Manuscript fine-tuning camera learning rate
    eps=1e-8,  # Default epsilon
    weight_decay=0  # Default weight decay
)
...
data_config.data = Path('D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanSeg_singlePodMASK')
...
###
# data_config_eval = NerfstudioDataParserConfig()
# data_config_eval.data = Path('data/unused_fruit_aisle')
# data_config_eval.downscale_factor = 1
# data_config_eval.train_split_fraction = 1
# #
# dataparser_eval = Nerfstudio(data_config_eval)
# ## dataparser
...
dataManager_config.dataparser.data = Path("D:/InvNerf_seg_relatedScripts/soybeanRelated/soybeanSeg_singlePodMASK")
...
# dataparser_train = dataparser._generate_dataparser_outputs('train')
# train_rays = dataparser_train.cameras.generate_rays(0)#origins,directions,pixel area,camera indices
# train_rays.origins.shape
# train_rays.directions.shape
# train_rays.camera_indices.shape
# train_rays.pixel_area.shape
#
# dataparser = Nerfstudio(data_config)
...
# eval_rays = dataparser_outputs.cameras.generate_rays(10)#origins,directions,pixel area,camera indices
# eval_rays.origins.shape
# eval_rays.directions.shape
# eval_rays.camera_indices.shape
# eval_rays.pixel_area.shape
...
for epoch in range(6000):
    ...
    if loss_out_final_i_0 < best_loss_i:
        ...
        save_checkpoint_multiple(nerfmodel, optimizers, 
                                 epoch, "soybean_singlepod_invnerf_128.pth")
    ...
    if (epoch +1)%100 ==0:
        ...
        #
        # if epoch >100:
        #     checkpoint_path = "unused_example_checkpoint.pth"
        #     load_checkpoint_multiple(checkpoint_path, nerfmodel, )
        nerfmodel.eval()
        with torch.no_grad():
            ...
            print("psnr_val {}".format(psnr_val))
                ...
                #     save_checkpoint_multiple(nerfmodel, optimizers, schedulers,epoch, "unused_example_checkpoint.pth")
