...
class NerfactoModel(Model):
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        # )
        loss_dict["semantics_loss"] = self.config.semantic_loss_weight * self.rgb_loss(
            torch.sigmoid(outputs["semantics"]), batch["fruit_mask"]
        )
        if self.training:
...
for folder_i in folder_ls:
    ...
    file_path = '{}/transforms.json'.format(folder_i)  # Update with the path to your file
    ...
    for image_name in img_ls_out:
        img_path = os.path.join("{}/images".format(folder_i), image_name)
        ...
        mask_path = os.path.join("{}/semantics".format(folder_i), mask_name)
    ...
    # checkpoint_path = "best_checkpoint_nerfacto{}FruitNeRF_April2026_defaultConfig_001_mseMaskLoss.pth".format(folder_i)
    ...
    for epoch in range(12000):
        ...
        if loss_out_final_i_0 < best_loss_i:
            ...
            save_checkpoint_multiple(nerfmodel, optimizers, 
                                     epoch, "best_checkpoint_nerfacto{}FruitNeRF_April2026_defaultConfig_001_mseMaskLoss.pth".format(folder_i))
        ...
        if (epoch)%50 ==0:
            ...
            with torch.no_grad():
                ...
                pred_mask_binary = pred_mask_out_n.float()
    ...
    import pickle
    with open("iou_log_fruitNerf_{}_mseLossOnly.pkl".format(folder_i), "wb") as f:
    ...
    plt.savefig('{}_ablation_fruitNeRF_mseLossOnly.png'.format(folder_i), dpi=300, bbox_inches='tight')
    plt.savefig('{}_ablation_fruitNeRF_mseLossOnly.pdf'.format(folder_i), dpi=300, bbox_inches='tight')
    plt.close()
