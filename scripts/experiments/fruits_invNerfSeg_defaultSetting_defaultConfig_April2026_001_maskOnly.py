...
for tr_stg in [1]:
    ...
    for f_i in range(len(folder_ls)):
        ...
        if tr_stg != 1:
            ...
            checkpoint_path = "best_checkpoint_invNerf_fruitsN{}_RGB_defaultConfig_001_April2026_MaskOnly.pth".format(folder_i_0)
            load_checkpoint_multiple(checkpoint_path, nerfmodel)
            # ### Freeze density MLP, only fine-tune color
            # for name, param in nerfmodel.field.named_parameters():
            #     if 'mlp_base' in name or 'position_encoding' in name:
        ...
        for epoch in range(1200):
            ...
            if loss_out_final_i_0 < best_loss_i:
                ...
                save_checkpoint_multiple(nerfmodel, optimizers, 
                                         epoch, "best_checkpoint_invNerf_fruitsN{}_RGB_defaultConfig_001_April2026_MaskOnly.pth".format(folder_i))
...
import pickle
with open("iou_log_invNerf_{}_001_maskOnly.pkl".format(folder_i), "wb") as f:
...
ax[1].imshow(pred_depth_out_n)
ax[1].set_xlabel('(b)', fontsize=16)
ax[1].set_xticks([])
ax[1].set_yticks([])

ax[2].imshow(gt_rgb)
ax[2].set_xlabel('(c)', fontsize=16)
ax[2].set_xticks([])
ax[2].set_yticks([])


plt.tight_layout(pad=1.0)
plt.savefig('{}_invNerf_ablation_002_maskOnly.png'.format(folder_i), dpi=300, bbox_inches='tight')
plt.savefig('{}_invNerf_ablation_002_maskOnly.pdf'.format(folder_i), dpi=300, bbox_inches='tight')
plt.close()
