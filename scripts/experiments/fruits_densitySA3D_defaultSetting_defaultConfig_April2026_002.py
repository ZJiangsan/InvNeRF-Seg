...
for folder_i in folder_ls:
    ...
    with torch.no_grad():
        for batch_i in range(NUM_BATCHES):
            ...
            mask_den = density >= 70
            combined_mask = mask_den.sum(dim=1).to(bool)
            ...
            if batch_i % 20 == 0:
                ...
                for e_a_i in zip(e_i_ls, a_i_ls):
                    ...
                    ax.set_ylabel('Y')
                    ax.set_zlabel('Z')
                    plt.title(f"SA3D (density) batch {batch_i}")
    ...
    o3d.io.write_point_cloud(f'variantSA3D_{folder_i}_densitySeg_002_defaultConfig.ply', pcd)
