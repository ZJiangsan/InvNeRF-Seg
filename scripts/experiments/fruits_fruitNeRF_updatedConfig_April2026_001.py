...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  7 10:36:13 2025
...
def convert_png_to_tiff_binary(input_dir: Path, output_dir: Path):
    """
    Converts .png binary masks to .tiff files with pixel values explicitly set to 0 and 1.

    Args:
        input_dir (Path): Directory containing the .png mask files.
        output_dir (Path): Directory to save the converted .tiff files.
    """
    ...
    for png_file in input_dir.glob("*.png"):
        ...
        with Image.open(png_file) as img:
            # Convert to grayscale (L mode ensures single channel)
            # grayscale_img = img.convert("L")

            # # Ensure binary values (0 or 1)
            # binary_array = np.array(grayscale_img) > 127  # Boolean array
            # binary_array = binary_array.astype(np.uint8)  # Convert to 0 and 1
            img_array = np.array(img)
            binary_array = img_array[:,:,0]
            binary_array[binary_array>0] = 1.0
            # print("max value = {}".format(np.max(binary_array)))
            # Convert back to an image
            binary_img = Image.fromarray(binary_array)

            # Save the binary image as .tiff
            output_path = output_dir / f"{png_file.stem}.tiff"
            binary_img.save(output_path, format="TIFF")

    print(f"Conversion completed. Files saved to {output_dir}")


file_path = 'appleTree/transforms.json'  # Update with the path to your file
transforms_data = load_transforms(file_path)
transforms_data.keys()
transforms_data['semantics']
...
for image_name in img_ls_out:
    img_path = os.path.join("appleTree/images", image_name)
    ...
    #
    mask_name = image_name#.replace('png', "tiff")
    mask_path = os.path.join("appleTree/semantics", mask_name)
    mask_i = cv2.imread(mask_path)
...
# torch.from_numpy(mask_array).unsqueeze(-1).bool()
...
# checkpoint_path = "best_checkpoint_nerfactoAppleFruitNeRF_April2026_updatedConfig_001.pth"
...
# field_scheduler = torch.optim.lr_scheduler.ExponentialLR(
#     optimizer=field_optimizer,
#     gamma=0.99  # Decay factor (final LR / initial LR)
# )
...
# camera_scheduler = torch.optim.lr_scheduler.ExponentialLR(
#     optimizer=camera_optimizer,
#     gamma=0.99  # Decay factor (final LR / initial LR)
# )
...
# schedulers = {
#     "field": field_scheduler,
#     "camera": camera_scheduler,
#     "proposal_networks": proposal_schedulers,
# }

images = torch.tensor(images_out)
masks = torch.tensor(masks_out)
##
...
data_config.data = Path('appleTree')
...
dataManager_config.dataparser.data = Path("appleTree")
...
for epoch in range(10000000):
    ...
    batch['fruit_mask'] = batch_train['fruit_mask'][:,:1].float().cuda()
    ...
    ##
    rgb_out_final_i_0 = loss_out['rgb_loss'].item()
    rgb_out_final_i += rgb_out_final_i_0
    #
    mask_out_final_i_0 = loss_out['semantics_loss'].item()
    mask_out_final_i += mask_out_final_i_0
    #
    loss_out_final_i_0 = rgb_out_final_i_0 + mask_out_final_i_0
    ##
    if loss_out_final_i_0 < best_loss_i:
        best_loss_i = loss_out_final_i_0
        save_checkpoint_multiple(nerfmodel, optimizers, 
                                 epoch, "best_checkpoint_nerfactoAppleFruitNeRF_April2026_updatedConfig_001.pth")
    ...
    if (epoch +1)%200 ==0:
        ...
        with torch.no_grad():
            # img_id = 20
            ray_bundle_test, batch_test = dataManager.next_eval_image(0)
            ...
            new_masks_n = batch_test['fruit_mask'][:,:,:].reshape(-1,1)
            # test_ind = 25
            ...
            for w_ii in range(height):
                ...
                batch_test_i['fruit_mask'] = new_masks_n[width*w_ii:(w_ii+1)*width,:].cuda()
                ##
                ...
                pred_mask = outputs_test['semantics'].clone().detach().cpu().reshape(1,width,1)#.reshape(image_size[1], image_size[0], 3)
                ...
                pred_mask_out.append(pred_mask)
                pred_depth_out.append(pred_depth)
            #
            pred_rgb_out_n = torch.cat(pred_rgb_out,0)
            pred_mask_out_n = torch.cat(pred_mask_out,0)
            ...
            gt_rgb = batch_test['image']
            ##
            gt_rgb = batch_test['image'][:,:,:3]
            gt_mask = batch_test['fruit_mask'][:,:,:]
            ##
            fig, ax = plt.subplots(1, 5, figsize=(16,4))
            ax[0].imshow(pred_rgb_out_n)
            ax[1].imshow(pred_mask_out_n)
            ax[2].imshow(pred_depth_out_n)
            ax[3].imshow(gt_rgb)
            ax[4].imshow(gt_mask)
            plt.show()
            #
            ...
            # ##
            # if loss_out['rgb_loss'].item() < best_loss and epoch >1:
            #     print("saving model at psnr of {}".format(psnr_val))
            #     best_loss = loss_out['rgb_loss'].item()
            #     save_checkpoint_multiple(nerfmodel, optimizers, schedulers,epoch, "best_checkpoint_nerfacto_mango_singleCameraIndice_001.pth")


# current epoch 18378
# rgb_loss: 0.006331041920930147
# semantics_loss: 0.07208919525146484
# interlevel_loss: 0.003409787081182003
# distortion_loss: 3.5605022276286036e-05
# orientation_loss: 6.872909125377191e-06
# pred_normal_loss: 0.00038838773616589606
# camera_opt_regularizer: 4.785601413459517e-05

# Total Loss: 0.07796823978424072
# current epoch 83637
# rgb_loss: 0.005191515665501356
# semantics_loss: 0.06379946321249008
# interlevel_loss: 0.0036036416422575712
# distortion_loss: 3.1218176445690915e-05
# orientation_loss: 6.634449164266698e-06
# pred_normal_loss: 0.0004299006250221282
# camera_opt_regularizer: 2.6723248083726503e-05
# Total Loss: 0.07308909296989441
