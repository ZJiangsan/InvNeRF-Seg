...
folder_ls = ["appleTreeSeg", "peachTreeSeg"]
# folder_ls = ["appleTreeSeg"]


for f_i in range(len(folder_ls)):
    folder_i = folder_ls[f_i]
    print("folder_i = {}".format(folder_i))
    folder_path = os.path.join(os.getcwd(), folder_i)    
    ...
    # "fineTuneColorFreezeDensity", "fineTuneDensityFreezeColor", ""
    # ablation_tag = "fineTuneDensityFreezeColor"
    # checkpoint_path = "best_checkpoint_invNerf_fruitsN{}_RGB_defaultConfig_001_April2026_{}.pth".format(folder_i, ablation_tag)
    # load_checkpoint_multiple(checkpoint_path, nerfmodel)
    # ##
    # point_path = '{}_invNerf_density_001_April2026_{}.ply'.format(folder_i,ablation_tag)
    ##
    checkpoint_path = "best_checkpoint_invNerf_fruitsN{}_RGB_defaultConfig_001_April2026.pth".format(folder_i)
    load_checkpoint_multiple(checkpoint_path, nerfmodel)
    ##
    point_path = '{}_invNerf_density_002_April2026.ply'.format(folder_i)
    ##    
    NUM_BATCHES = 500
    points_den = []
    ...
    with torch.no_grad():
        for batch_i in range(NUM_BATCHES):
            ...
            rgb_brightness = density_color.mean(dim=-1)
            fruit_mask = rgb_brightness > 0.65
            
            points_3d_density = points_3d_density[fruit_mask]

            if points_3d_density.numel() == 0:
                continue
