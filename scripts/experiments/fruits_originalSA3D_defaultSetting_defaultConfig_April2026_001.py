...
class NerfactoModel(Model):
    ...
    def get_export_outputs(self, ray_bundle: RayBundle):
        ...
        outputs["weights"] = weights[..., 0]  # <-- add this
...
folder_i = "peachTree" # "appleTree", "peachTree"
...
with progress as progress_bar:
    ...
    while not progress_bar.finished:
        with torch.no_grad():
            ...
            # --- Weight-based SA3D (original) ---
            weight_0 = outputs['weights'].reshape((-1, 1)).repeat((1, 3))
            weight = weight_0[mask_bbox]
            mask_weight = weight >= 0.1  # threshold on rendering weights, tune as needed
            ...
            # RGB
            points_3d_density = points_3d[mask_weight.sum(dim=1).to(bool)]
            if rgb_flag:
                density_color = rgb[mask_weight.sum(dim=1).to(bool)]
            else:
                density_color = density[mask_weight.sum(dim=1).to(bool)]
            density_color = torch.hstack(
                [density_color, torch.sigmoid(density[mask_weight.sum(dim=1).to(bool)][:, 0]).unsqueeze(-1)])
            #
            # density semantic
            points_3d_density_sem = points_3d[mask_weight.sum(dim=1).to(bool)]
            den_sem = True
            if den_sem:
                density_sem = semantic[mask_weight.sum(dim=1).to(bool)]
            else:
                density_sem = density[mask_weight.sum(dim=1).to(bool)]
            
            density_sem = torch.hstack(
                [density_sem, torch.sigmoid(mask_weight[mask_weight.sum(dim=1).to(bool)][:, 0]).unsqueeze(-1)])

            # rgb_color = rgb[mask_weight.sum(dim=1).to(bool)]
    ...
    pcd_list.update({'density_sem': {
        'pcd': pcd,
        'path': str('originalSA3D_{}_weightSeg_001_defaultConfig.ply'.format(folder_i))
    }})
    ...
    pcd_list.update({'density': {
            'pcd': pcd,
            'path': str('originalSA3D_{}_weightColor_001_defaultConfig.ply'.format(folder_i))
        }})
