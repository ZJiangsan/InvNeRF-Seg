...
for folder_i in folder_ls:
    # folder_i = "appleTree" # "appleTree", "peachTree"
    ...
    with torch.no_grad():
        for batch_i in range(NUM_BATCHES):
            ...
            ray_bundle = RayBundle(
                ...
                camera_indices=ray_bundle_train.camera_indices.long().cuda()
            )
            outputs = nerfmodel.get_export_outputs(ray_bundle)
    
            min_bound = torch.tensor([-0.5, -0.5, -0.7]).cuda()
            ...
            mask_weight = weight >= 0.1
    
            # Fruit points: weight threshold + semantic mask
            combined_mask = mask_weight.sum(dim=1).to(bool)
            points_3d_filtered = points_3d[combined_mask]
            semantic_filtered = semantic[combined_mask]
    
            # Keep only fruit (semantic == 1)
            fruit_mask = semantic_filtered[:, 0] > 0.65
            fruit_points = points_3d_filtered[fruit_mask]
    
            if fruit_points.numel() == 0:
                continue
    
            points_den_sem.append(fruit_points.cpu())
            ...
            if batch_i % 20 == 0:
                ...
                for e_a_i in zip(e_i_ls, a_i_ls):
                    ...
                    x, y, z = fruit_points.cpu()[:, 0], fruit_points.cpu()[:, 1], fruit_points.cpu()[:, 2]
                    ax.scatter(x, y, z, s=1)
                    ax.set_xlabel('X')
                    ax.set_ylabel('Y')
                    ax.set_zlabel('Z')
                    plt.title(f"SA3D (weight) batch {batch_i}")
                    ax.view_init(elev=e_a_i[0], azim=e_a_i[1])
                    plt.show()
    
            torch.cuda.empty_cache()
    
    # Save as white points
    points_den_sem = torch.cat(points_den_sem, dim=0)
    white = np.ones((points_den_sem.shape[0], 3), dtype=np.float64)
    
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points_den_sem.double().cpu().numpy())
    pcd.colors = o3d.utility.Vector3dVector(white)
    o3d.io.write_point_cloud(f'originalSA3D_{folder_i}_weightSeg_002_defaultConfig.ply', pcd)
